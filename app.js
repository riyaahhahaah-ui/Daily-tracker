document.addEventListener('DOMContentLoaded', () => {
    ...
    const historySection = document.querySelector('.history-card');
    const demoBtn = document.createElement('button');
    demoBtn.innerText = "Generate Demo History";
    demoBtn.style.cssText = "margin-top: 10px; font-size: 0.8rem; background: none; border: 1px dashed #aaa; padding: 5px; cursor: pointer; opacity: 0.6;";
    historySection.appendChild(demoBtn);
    ...
    function saveData() {
        const storageKey = `dailyTracker_${currentState.date}`;
    ...
    function calculateWeeklyStats() {
        const keys = Object.keys(localStorage)
            .filter(key => key.startsWith('dailyTracker_'))
            .sort()
            .reverse()
    ...
    function loadHistory() {
        ...
        keys.forEach(key => {
            try {
                const entry = JSON.parse(localStorage.getItem(key));
                // Extract date part: dailyTracker_2023-10-27
